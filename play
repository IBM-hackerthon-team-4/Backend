...
def get_image_urls(search_term):
    print(f"Searching for image URLs with keyword: {search_term}")
    google_api_key = "AIzaSyAakKrTq1NikGxyEfX678VEytg8x25BEho"
    google_cx = "461254d9dc19a4cf0"
    url = f"https://www.googleapis.com/customsearch/v1?key={google_api_key}&cx={google_cx}&q={search_term}"
    try:
        ...
        for idx, url in enumerate(image_urls, start=1):
            ...
            try:
                ...
                if data:
                    ...
                    print("no data returned from watson, db insert skipping.")
                time.sleep(1)
            except Exception as e:
